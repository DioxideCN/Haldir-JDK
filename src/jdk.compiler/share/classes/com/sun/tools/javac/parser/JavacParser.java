...
/* ---------- parsing -------------- */
        ...
        case SWITCH:
            ...
            while (true) {
                ...
                case RBRACE: case EOF:
                    JCSwitchExpression e = to(F.at(switchPos).SwitchExpression(selector, cases.toList()));
                    e.endpos = token.pos;
                    accept(RBRACE);
                    return e;
                default:
    ...
    List<JCStatement> blockStatement() {
        ...
        Comment dc;
        int pos = token.pos;
        switch (token.kind) { // switch start
        case RBRACE: case CASE: case DEFAULT: case EOF:
            return List.nil();
        case LBRACE: case IF: case FOR: case WHILE: case DO: case TRY:
        case SWITCH: case SYNCHRONIZED: case RETURN: case THROW: case BREAK:
        case CONTINUE: case SEMI: case ELSE: case FINALLY: case CATCH:
        case ASSERT: case RETRIAL: // assert, retrial, for, if and others
            return List.of(parseSimpleStatement());
        case MONKEYS_AT:
        case FINAL: { // final annotation starter of @
            dc = token.comment(CommentStyle.JAVADOC);
            JCModifiers mods = modifiersOpt();
            if (token.kind == INTERFACE || // interface
                token.kind == CLASS || // class
                token.kind == ENUM || // enum
                isRecordStart()) { // record
        ...
        case ABSTRACT: case STRICTFP: { // abstract strictfp
            dc = token.comment(CommentStyle.JAVADOC);
            JCModifiers mods = modifiersOpt();
            return List.of(classOrRecordOrInterfaceOrEnumDeclaration(mods, dc));
        }
        ...
        case ENUM: // enum
            ...
            dc = token.comment(CommentStyle.JAVADOC);
            return List.of(classOrRecordOrInterfaceOrEnumDeclaration(modifiersOpt(), dc));
        case IDENTIFIER: // IDENTIFIER(Tag.NAMED) such as yield
    ...
    public JCStatement parseSimpleStatement() {
        ...
        case TRY: {
            ...
            if (token.kind == LPAREN) { // try-resource `try() {}`
                nextToken();
                resources = resources();
                accept(RPAREN);
            }
            ...
            // resolve `catch` and `finally`
            if (token.kind == CATCH || token.kind == FINALLY) {
                // first resolve catch chain, append Exceptions to List
                while (token.kind == CATCH) catchers.append(catchClause());
                if (token.kind == FINALLY) { // then resolve finally
                    nextToken(); // move to next token `{`
                    finalizer = block(); // here get `{}` from `finally {}`
                }
            } else {
                if (resources.isEmpty()) { // empty resource
                    log.error(DiagnosticFlag.SYNTAX, pos, Errors.TryWithoutCatchFinallyOrResourceDecls);
                }
            }
            // send <resource> <block> <exceptions> <finally>
        ...
        case RETRIAL: {
            ...
            accept(LPAREN); // get condition block `()`
            ...
            JCExpression loopCondition = F.Binary(JCTree.Tag.AND, condition, F.Binary(JCTree.Tag.LT, F.Ident(loopCounterName), timesExpr));
    ...
    loop:
        while (true) {
            long flag;
            switch (token.kind) {
            case PRIVATE     : flag = Flags.PRIVATE; break; // private
            case PROTECTED   : flag = Flags.PROTECTED; break; // protected
            case PUBLIC      : flag = Flags.PUBLIC; break; // public
            case STATIC      : flag = Flags.STATIC; break; // static
            case TRANSIENT   : flag = Flags.TRANSIENT; break; // transient
            case FINAL       : flag = Flags.FINAL; break; // final
            case ABSTRACT    : flag = Flags.ABSTRACT; break; // abstract
            case NATIVE      : flag = Flags.NATIVE; break; // native
            case VOLATILE    : flag = Flags.VOLATILE; break; // volatile
            case SYNCHRONIZED: flag = Flags.SYNCHRONIZED; break; // synchronized
            case STRICTFP    : flag = Flags.STRICTFP; break; // strictfp
            case MONKEYS_AT  : flag = Flags.ANNOTATION; break; // @
            case DEFAULT     : checkSourceLevel(Feature.DEFAULT_METHODS); flag = Flags.DEFAULT; break; // default
            case ERROR       : flag = 0; nextToken(); break; // ERROR()
            case IDENTIFIER  : { // IDENTIFIER(Tag.NAMED)
        ...
        switch (token.kind) {
        case ENUM: flags |= Flags.ENUM; break; // enum
        case INTERFACE: flags |= Flags.INTERFACE; break; // interface
        default: break;
        }
    ...
    /**
     * here is the entrance of java file resolver
     * CompilationUnit = [ { "@" Annotation } PACKAGE Qualident ";"]
     * {ImportDeclaration}
     * {TypeDeclaration}
     */
    public JCTree.JCCompilationUnit parseCompilationUnit() {
        Token firstToken = token; // first token
        JCModifiers mods = null; // like static abstract interface ...
        boolean consumedToplevelDoc = false;
        boolean seenImport = false;
        boolean seenPackage = false;
        ListBuffer<JCTree> defs = new ListBuffer<>();
        if (token.kind == MONKEYS_AT) // 1. resolve annotation type
            mods = modifiersOpt(); // set modifiers

        if (token.kind == PACKAGE) { // 2. resolve package type
            int packagePos = token.pos;
            List<JCAnnotation> annotations = List.nil();
            seenPackage = true; // ready to scan package
            if (mods != null) {
                checkNoMods(mods.flags & ~Flags.DEPRECATED); // @Deprecated
                annotations = mods.annotations;
                mods = null;
            }
            nextToken(); // move to nextt token
    ...
    JCTree typeDeclaration(JCModifiers mods, Comment docComment) {
        ...
        if (mods == null && token.kind == SEMI) {
            ...
            return classOrRecordOrInterfaceOrEnumDeclaration(modifiersOpt(mods), docComment); // class record interface enum
        }
    }
    ...
    protected JCStatement classOrRecordOrInterfaceOrEnumDeclaration(JCModifiers mods, Comment dc) {
        if (token.kind == CLASS) { // class
            return classDeclaration(mods, dc);
        } if (isRecordStart()) { // record
            return recordDeclaration(mods, dc);
        } else if (token.kind == INTERFACE) { // interface
            return interfaceDeclaration(mods, dc);
        } else if (token.kind == ENUM) { // enum
    ...
    protected JCClassDecl classDeclaration(JCModifiers mods, Comment dc) {
        ...
        if (token.kind == EXTENDS) { // extends
            nextToken();
            extending = parseType();
        }
        List<JCExpression> implementing = List.nil();
        if (token.kind == IMPLEMENTS) { // implments
            nextToken();
